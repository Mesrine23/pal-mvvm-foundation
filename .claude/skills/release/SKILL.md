...
Releasing $ARGUMENTS. Work through this in order and stop at step 6 for the owner.
...
Newest first, with the `Affects:` line first inside it. The derivation command, the trimming rule, the entry template, and the voice all live in `.claude/rules/changelog.md`, which loads when you open `CHANGELOG.md`.

## 4. Update the status docs
...
## 5. Ripple the docs and the adopter kit
