...
>
> **`v1.5.2` shipped** (2026-08-23): documentation-only — two fixes to the agent tooling `v1.5.1` introduced, both caught by using it. CI now runs on `develop` pushes (it previously covered only `main` and PRs, so a release candidate could reach the tag unbuilt), and `/verify`'s branch-discipline condition no longer contradicts `/release`'s requirement to work on `develop`. No API or behavior change.
...
- **Polish pass (post-POC) — status docs were stale.** `CLAUDE.md` "Current status" and the phase list still read "Phase 0 / empty stubs" at `v0.9.0` — caught by an early adoption agent. Fixed. **Process rule: update status docs every phase.**
...
- **`v1.5.0` (branch) — a deliberate source break shipped in a MINOR, pre-adoption.** The end-to-end validation of PalNetworking against a local mock backend (journal-verified assertions over real HTTP) demanded response headers on error responses — the gate for `Retry-After`/429 handling, `Location` on denied redirects, and rate-limit headers. That means `unacceptableStatus(code:data:)` → `unacceptableStatus(code:data:headers:)`, which is source-breaking for pattern matches (SemVer says major). **Owner ruling: ship it in `v1.5.0`** — PalNetworking has zero adopters, so a `v2.0.0` would version-tax an unused surface; the break costs one `_` per match site, all in-repo and fixed in the same commit. The `api-stability` gate is expected red on the branch and re-baselines at the release tag (its documented deliberate-break flow). A second, smaller instance of the same ruling: `RequestOptions.init` was extended in place with defaulted parameters instead of duplicating the old signature (the `ThemeColors` precedent) — call sites stay source-compatible; only the exact old signature disappears. Companion redaction work: with headers riding in the error, `LoggingInterceptor`'s public-privacy `String(describing: error)` would have leaked them — `NetworkError` gained a redaction-safe `CustomStringConvertible` (counts, never values), closing the hole at the type rather than at every log site.
- **Validation discovery — URLSession transparently retries dropped connections.** With the mock backend destroying sockets on purpose, ONE logical `send()` produced up to 3 server hits before PalNetworking saw any error, and `RetryInterceptor(maxRetries: 2)` produced up to 9 — the OS retries beneath the interceptor. Not a Pal bug and not controllable from this layer; the cap governs *logical* attempts. Documented in the product guide, the `RetryInterceptor` docs, and here, so server-side rate-limit/idempotency budgets aren't sized to the wrong number.
- **`build-and-test-latest` flake — timing-based tests race the runtime scheduler.** After the `macos-26` image moved to **Swift 6.3.2**, `LoaderTests."Loading keeps the previous value"` failed: it did a fire-and-forget `loader.load { "first" }`, slept a fixed **40 ms**, then asserted `.loaded("first")` — the newer scheduler hadn't run the load's background `Task` yet (a 50 ms sibling test passed, confirming pure timing). No product regression. Fix: the Loader tests no longer guess a sleep — they `performLoad` (awaited) where they can, rely on `load(_:)`'s **synchronous** `.loading(previous:)` transition for re-trigger checks, and otherwise `waitUntil { … }`-poll the observable state (generous timeout). **Process lesson: never assert async completion after a fixed `Task.sleep` — poll the state or await a completion signal, so tests survive scheduler/runtime drift across toolchains.**
