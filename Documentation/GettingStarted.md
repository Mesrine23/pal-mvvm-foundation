...
> The first-load `LoadingView()` can be a **skeleton** instead — render your real rows with placeholder values and `.skeleton(when: true)` (see [PalDesignSystem](Products/PalDesignSystem.md)); the Example's Users and Posts screens do exactly that.
...
- **Reachability** — one `ReachabilityMonitor` at the composition root; read `status` for an offline banner or observe `statusUpdates`. UX affordances only — never a request preflight gate. See [PalNetworking](Products/PalNetworking.md).
- **Biometrics** — `BiometricAuthenticator` for Face ID / Touch ID gating; cancellation is an outcome, never an error; remember `NSFaceIDUsageDescription`. See [PalAuth](Products/PalAuth.md).
...
### Option C — Track a branch/commit — pre-release co-development only

Pin the dependency to a **branch** (e.g. `develop`) or a specific commit instead of a version, push to the foundation, and *Update Packages*. No tag per tweak. This is for the tight loop while co-developing an unreleased foundation change — **consumers on released Pal track tags, never branches** (the SemVer contract below); switch back to a version pin as soon as the change ships.
