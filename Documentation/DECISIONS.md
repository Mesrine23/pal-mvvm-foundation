...
- **DS additions (`v1.4.0`, additive — BarRoster feedback):** `surfaceElevated` color slot (floating chrome: sheets/alerts/toasts; **defaults to `surface`** so existing themes render identically — the alert/toast cards now draw from it) · `Color(hex:)` (UInt32 compile-checked + failable `#RRGGBB`/`#RRGGBBAA` string — every token layer was hand-rolling it) · `FlowLayout` (wrapping chip rows — the components bar of "wait for dogfooding demand" was met by two independent apps writing the same layout). *Evolution lesson, caught by the `api-stability` gate: inserting a defaulted parameter into an existing `init` is source-compatible but **removes the old signature** — the pre-`v1.4.0` `ThemeColors.init` is kept verbatim alongside the new one.*
